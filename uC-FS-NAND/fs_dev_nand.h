...
//#if (FS_NAND_INTERN_API_EN == DEF_ENABLED)
    //#if (FS_NAND_INTERN_ACCESS == DEF_NO)
        const  CPU_CHAR  *FS_NAND_NameGet (void);
        void  FS_NAND_Init (FS_ERR  *p_err);
        void FS_NAND_Open (FS_DEV  *p_dev, void    *p_dev_cfg, FS_ERR  *p_err);
        void  FS_NAND_Close (FS_DEV  *p_dev);
        void FS_NAND_Rd (FS_DEV      *p_dev,
                                  void        *p_dest,
                                  FS_SEC_NBR   sec_start,
                                  FS_SEC_QTY   sec_cnt,
                                  FS_ERR      *p_err);
        void  FS_NAND_Wr (FS_DEV      *p_dev,
                                        void        *p_src,
                                        FS_SEC_NBR   sec_start,
                                        FS_SEC_QTY   sec_cnt,
                                        FS_ERR      *p_err);
        void FS_NAND_Query (FS_DEV       *p_dev,
                                            FS_DEV_INFO  *p_info,
                                            FS_ERR       *p_err);
        void  FS_NAND_IO_Ctrl (FS_DEV      *p_dev,
                                            CPU_INT08U   opt,
                                            void        *p_data,
                                            FS_ERR      *p_err); 
    //#endif
