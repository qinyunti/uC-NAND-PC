    ...
    MEM_VAL_COPY_SET_INTU_LITTLE(&p_hdr_data[FS_NAND_HDR_MAX_BAD_BLK_CNT_OFFSET],
                                 &max_bad_blk_cnt_hdr,
                                  sizeof(FS_NAND_HDR_MAX_BAD_BLK_CNT_TYPE));
                                                                ...
                                                                /* ------------------ CALC OOS DATA ------------------- */
    memset(&p_oos_data[FS_NAND_OOS_SEC_USED_OFFSET],           /* Insert sec used mark.                                */
             0x00u,
                                            ...
                                            FS_ERR        *p_err)
{
               ...
               *p_err = FS_ERR_NONE;
                ...
                if (status_finished == DEF_YES) {               /* Seq finished; we can consider sec.                   */
                    ...
                    if ((p_nand_data->MetaBlkInvalidSecMap[offset_octet] & DEF_BIT(offset_bit)) != 0u) {
                        ...
                        //         &p_sec_buf[0],
                        //         copy_size);
												memcpy(&p_nand_data->MetaCache[meta_sec_ix * p_nand_data->SecSize],
                                 &p_sec_buf[0],
                                 copy_size);
    ...
    memcpy(&p_nand_data->MetaBlkInvalidSecMap[0],
             &meta_map[0],
              map_size);
                                          ...
                                          FS_ERR        *p_err)
{
    ...
    if (avail_tbl_only == DEF_NO) {
        memcpy(&p_nand_data->DirtyBitmapCache[0u],
                                             ...
                                             FS_ERR                   *p_err)
{
    ...
    memset(&p_oos_buf[FS_NAND_OOS_SEC_USED_OFFSET],            /* Sec used mark.                                       */
             0x00u,
             p_nand_data->UsedMarkSize);
                                                 ...
                                                 CPU_INT08U            *p_buf)
{
    ...
    memcpy(p_buf, &p_metadata[offset_octet], size);           /* Copy data.                                           */
}
#endif
                                       ...
                                       FS_ERR           *p_err)
{
    ...
    FS_NAND_UB_QTY            ub_ix;
    FS_NAND_UB_DATA           entry;
    FS_NAND_UB_EXTRA_DATA    *p_entry_extra = NULL;
    FS_NAND_SEC_PER_BLK_QTY   sec_offset_phy;
    FS_NAND_BLK_QTY           blk_ix_logical;
    FS_NAND_SEC_PER_BLK_QTY   sec_offset_logical;
                                      ...
                                      FS_NAND_UB_QTY   ub_ix)
{
    ...
    memset(&ub_data.SecValidBitMap[0], 0, size);
    ...
    while (rd_retries < FS_NAND_CFG_MAX_RD_RETRIES) {           /* Loop until rd success or retry cnt exceeded.         */
        ...
        if ((*p_err == FS_ERR_ECC_CORR) ||
           ...
           *p_err = FS_ERR_NONE;
            ...
            set_bit_cnt = NAND_CRCUtil_PopCnt_32(sec_used_mark);
                                         ...
                                         FS_ERR                   *p_err)
{
    ...
    memset(&p_oos_buf_08[FS_NAND_OOS_SEC_USED_OFFSET],         /* Sec used mark.                                       */
             0x00u,
             p_nand_data->UsedMarkSize);
                                           ...
                                           FS_ERR        *p_err)
{
    ...
    nbr_blk -= p_nand_data->AvailBlkTblEntryCntMax;             /* ��������AvailBlkTblEntryCntMax����ʵ�ʵİ�, FS_NAND_CFG_RSVD_AVAIL_BLK_CNT+1 ֻ����Сֵ? ʵ������ֵ��AvailBlkTblEntryCntMax */
                                            ...
                                            FS_ERR        *p_err)
{
    ...
    for (tbl_ix = 0u; tbl_ix < p_nand_data->UB_CntMax; tbl_ix++) {
        ...
        memset(&p_tbl_entry->AssocLogicalBlksTbl[0], 0xFFu, sizeof(FS_NAND_BLK_QTY) * p_nand_data->RUB_MaxAssoc);
        ...
        if (ub_sec_map_size != 0u) {                            /* If size if non-null ...                              */
            ...
            memset(&p_tbl_entry->LogicalToPhySecMap[0], 0x00u, sizeof(CPU_INT08U) * ub_sec_map_size);
        } else {
...
FS_NAND_INTERN  void  FS_NAND_InitDevData (FS_NAND_DATA  *p_nand_data)
{
    ...
    p_nand_data->DirtyBitmapSrchPos     = 0u;
                                                                ...
                                                                /* ------------------- INIT L2P TBL ------------------- */
    memset((void *)p_nand_data->LogicalToPhyBlkMap, 0xff, sizeof(FS_NAND_BLK_QTY) * p_nand_data->LogicalDataBlkCnt);

                                                                /* ---------------- INIT AVAIL BLK TBL ---------------- */
    p_buf = &p_nand_data->MetaCache[p_nand_data->MetaOffsetAvailBlkTbl];
    memset(&p_buf[0], 0xFFu, p_nand_data->MetaOffsetBadBlkTbl - 0u);

                                                                /* ----------------- INIT BAD BLK TBL ----------------- */
    p_buf = &p_nand_data->MetaCache[p_nand_data->MetaOffsetBadBlkTbl];
    memset(&p_buf[0], 0xFFu, p_nand_data->MetaOffsetDirtyBitmap - p_nand_data->MetaOffsetBadBlkTbl);


                                                                /* ---------------- INIT DIRTY BITMAP ----------------- */
    p_buf = &p_nand_data->MetaCache[p_nand_data->MetaOffsetDirtyBitmap];
    memset(&p_buf[0], 0x00u, p_nand_data->MetaOffsetUB_Tbl - p_nand_data->MetaOffsetDirtyBitmap);
    ...
    for (tbl_ix = 0u; tbl_ix < p_nand_data->UB_CntMax; tbl_ix++) {
        memset(&p_buf[tbl_ix * (blk_sec_bitmap_size + sizeof(FS_NAND_BLK_QTY))],
                 0xFFu,
                 sizeof(FS_NAND_BLK_QTY));
        memset(&p_buf[tbl_ix * (blk_sec_bitmap_size + sizeof(FS_NAND_BLK_QTY)) + sizeof(FS_NAND_BLK_QTY)],
                 0x00u,
                 blk_sec_bitmap_size);
    }
    ...
    //Mem_Clr((void *)p_nand_data->MetaBlkInvalidSecMap, meta_invalid_map_size);
		memset((void *)p_nand_data->MetaBlkInvalidSecMap, 0, meta_invalid_map_size);
    ...
    for (tbl_ix = 0u; tbl_ix < p_nand_data->UB_CntMax; tbl_ix++) {
        ...
        if (ub_sec_map_size != 0u) {                            /* If size if non-null ...                              */
            ...
            p_tbl_entry->LogicalToPhySecMap = DEF_NULL;
                                                                ...
                                                                /* See Note #1.                                         */
    memset(p_nand_data->AvailBlkMetaMap, 0xFF, FS_UTIL_BIT_NBR_TO_OCTET_NBR(p_nand_data->AvailBlkTblEntryCntMax));
}
...
FS_NAND_INTERN  FS_NAND_DATA  *FS_NAND_DataGet (void)
{
    ...
    //Mem_Clr(p_nand_data, sizeof(FS_NAND_DATA));
		memset(p_nand_data, 0, sizeof(FS_NAND_DATA));
