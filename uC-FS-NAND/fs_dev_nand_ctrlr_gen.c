                     ...
                     FS_ERR            *p_err)
{
    ...
    parity_manuf  = NAND_CRCUtil_PopCnt_32(id_manuf);                /* Calc parity of manuf id.                             */
                     ...
                     FS_ERR      *p_err)
{
    ...
    FS_ERR_CHK(p_bsp_api->DataRd(p_dest_oos, p_ctrlr_data->OOS_InfoTbl[sec_offset_pg].Len, p_part_data->BusWidth, p_err),
               p_bsp_api->ChipSelDis());
#endif
    p_bsp_api->SecRd(row_addr, col_addr, p_ctrlr_data->OOS_InfoTbl[sec_offset_pg].PgOffset, p_dest, p_ctrlr_data->SecSize, p_dest_oos, p_ctrlr_data->OOS_InfoTbl[sec_offset_pg].Len, p_err);
    if (*p_err != FS_ERR_NONE)
    { 
        return; 
    }
                        ...
                        FS_ERR           *p_err)
{
...
#endif
    p_spare_08  = (CPU_INT08U *)p_ctrlr_data->SpareBufPtr;
    p_bsp_api->SecRd(row_addr, 0, p_ctrlr_data->OOS_InfoTbl[sec_offset_pg].PgOffset, 0, 0 , p_spare_08, p_ctrlr_data->OOS_InfoTbl[sec_offset_pg].Len, p_err);
    if (*p_err != FS_ERR_NONE)
    { 
        return; 
    }
    ...
    memcpy(p_dest_oos, p_spare_08, len);
                       ...
                       FS_ERR           *p_err)
{
    ...
    p_bsp_api->ChipSelDis();
#endif
    p_bsp_api->SecRd(pg_ix_phy, offset, 0, p_dest, len, 0, 0, p_err);
    if (*p_err != FS_ERR_NONE)
    { 
        return; 
    }
                     ...
                     FS_ERR      *p_err)
{
   ...
   *p_err = FS_ERR_NONE;
#endif
    p_bsp_api->SecWr(row_addr, col_addr, p_ctrlr_data->OOS_InfoTbl[sec_offset_pg].PgOffset, p_src, p_ctrlr_data->SecSize, p_src_oos, p_ctrlr_data->OOS_InfoTbl[sec_offset_pg].Len, p_err);
    if(*p_err != FS_ERR_NONE)
    {
        return;
    }
                        ...
                        FS_ERR      *p_err)
{
   ...
   *p_err = FS_ERR_NONE;
#endif
    p_bsp_api->BlkErase(blk_ix_phy,p_err);
}
                         ...
                         FS_ERR                            *p_err)
{
    ...
    p_bsp_api->ChipSelDis();
#endif
    p_bsp_api->ParamPageRd(p_param_pg->DataPtr, FS_NAND_PART_ONFI_PARAM_PAGE_LEN, p_err);
}
